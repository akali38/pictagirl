 ...
 */
 ...
 function achievement_mac_desc($text='',$type='',$points='',$times='',$title='',$data=''){
  ...
  if($times>1){
    ...
    $contest = __('contests', 'wpachievements');
    $prize = __('prizes', 'wpachievements');
  } else{
    $contest = __('contest', 'wpachievements');
    $prize = __('prize', 'wpachievements');
  }
  switch($type){
   case 'myarcadecontest_subgaming': { $text = sprintf( __('for playing %s %s', 'wpachievements'), $times, $contest); } break;
   case 'myarcadecontest_add': { $text = sprintf( __('for getting %s contest %s', 'wpachievements'), $times, $prize); } break;
   case 'myarcadecontest_subcontest': { $text = sprintf( __('for joining %s %s', 'wpachievements'), $times, $contest); } break;
  }
  return $text;
 }
 ...
 function quest_mac_desc($text='',$type='',$times=''){
  ...
  switch($type){
   case 'myarcadecontest_subgaming': { $text = sprintf( __('Play %s %s', 'wpachievements'), $times, $contest); } break;
   case 'myarcadecontest_add': { $text = sprintf( __('Get %s contest %s', 'wpachievements'), $times, $prize); } break;
   case 'myarcadecontest_subcontest': { $text = sprintf( __('Join %s %s', 'wpachievements'), $times, $contest); } break;
  }
  return $text;
 }
