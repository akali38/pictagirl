 ...
 */
 ...
 function wpachievements_admin_scripts($hook) {
  ...
  if( 'users.php' == $hook ){
    ...
    wp_enqueue_script( 'UI_Spinner_Script', WPACHIEVEMENTS_URL . '/assets/js/ui.spinner.js' );
  ...
  if( 'wpachievements_page_wpachievements_settings' != $hook && 'wpachievements_page_wpachievements_ranks' != $hook && 'wpachievements_page_wpachievements_achievements' != $hook && 'wpachievements_page_wpachievements_supported_plugins' != $hook && 'wpachievements_page_wpachievements_faq' != $hook && 'wpachievements_page_wpachievements_documentation' != $hook )
   return;
  ...
  if( 'toplevel_page_wpachievements_admin' == $hook ){
    ...
    wp_enqueue_style( 'support_admin_style', WPACHIEVEMENTS_URL . '/assets/css/support-admin.css' );
    ...
    wp_enqueue_style( 'SelectBox', WPACHIEVEMENTS_URL . '/assets/css/jquery.selectbox.css' );
    wp_enqueue_style( 'thickbox' );
    wp_enqueue_script( "UIScript", WPACHIEVEMENTS_URL . "/assets/js/ui-darkness/js/jquery-ui-1.10.3.custom.js" );
    wp_enqueue_script( 'UI_Spinner_Script', WPACHIEVEMENTS_URL . '/assets/js/ui.spinner.js' );
    wp_enqueue_script( 'SelectBox_Script', WPACHIEVEMENTS_URL . '/assets/js/jquery.selectbox-0.2.js' );
 ...
 function wpachievements_admin_menu(){
  ...
  add_menu_page(__("WPAchievements", 'wpachievements'), __("WPAchievements", 'wpachievements'), $user_role, 'edit.php?post_type=wpachievements', '', WPACHIEVEMENTS_URL . '/assets/img/logo_small.png' );

  if($rankstatus != 'Disable'){
    add_submenu_page( 'edit.php?post_type=wpachievements', __('WPAchievements - Ranks', 'wpachievements'), __('Ranks', 'wpachievements'), $user_role, 'wpachievements_ranks', 'wpachievements_ranks_admin');
  }
  ...
  /*add_submenu_page( 'edit.php?post_type=wpachievements', __('WPAchievements - Shortcodes', 'wpachievements'), __('Shortcodes', 'wpachievements'), 'manage_options', 'wpachievements_documentation', 'wpachievements_documentation');*/


  add_menu_page(__("WPQuests", 'wpachievements'), __("WPQuests", 'wpachievements'), $user_role, 'edit.php?post_type=wpquests', '', WPACHIEVEMENTS_URL . '/assets/img/logo_small.png' );
  ...
  add_submenu_page( 'edit.php?post_type=wpachievements', __('WPAchievements - Supported Plugins', 'wpachievements'), __('Supported Plugins', 'wpachievements'), 'manage_options', 'wpachievements_supported_plugins', 'wpachievements_supported_plugins');
...
function wpachievements_settings_admin(){
  require_once( 'admin/class-wpachievements-admin-settings.php' );
 ...
 */
 ...
 function wpachievements_how_box( $post ){
  ...
  if( function_exists(WPACHIEVEMENTS_LEARNDASH) ){
    ...
    echo '<span id="first_try" class="pullleft"'.$show.'>
      <label for="wpachievements_achievement_ld_first_try">'.__('First Attempt Only', 'wpachievements').':</label>
      <select id="wpachievements_achievement_ld_first_try" name="wpachievements_achievement_ld_first_try">';
        ...
        echo '<option value="Disabled">'.__('Disabled', 'wpachievements').'</option>
        <option value="Enabled">'.__('Enabled', 'wpachievements').'</option>
      </select>
    </span>';
  }
  ...
  <div id="event_details" style="display:none;">';
    ...
    if( !empty($cur_post) && ($cur_trigger == 'gform_sub' || $cur_trigger == 'ld_lesson_complete' || $cur_trigger == 'ld_course_complete' || $cur_trigger == 'ld_quiz_pass' || $cur_trigger == 'wpcw_quiz') ){
      if($cur_trigger == 'ld_lesson_complete' || $cur_trigger == 'ld_course_complete'){
        $postid_title = __('Lesson ID', 'wpachievements');
      }
      ...
      if($cur_trigger == 'wpcw_module_complete'){
        $postid_title = __('Module ID', 'wpachievements');
      }
      if($cur_trigger == 'wpcw_course_complete'){
        $postid_title = __('Course ID', 'wpachievements');
      }
      if($cur_trigger == 'gform_sub'){
        $postid_title = __('Form ID', 'wpachievements');
      }
      $show = ' style="display:block !important;"';
    } else{
      $postid_title = __('Form ID', 'wpachievements');
    ...
    if( class_exists(WPACHIEVEMENTS_WOOCOMMERCE) ){
      ...
      <span id="woo_order_limit"'.$show.'>
        <label for="wpachievements_achievement_woo_order_limit">'.__('Minimum Order Amount', 'wpachievements').': <small>(Optional)</small></label>
    ...
    <label for="wpachievements_achievements_data_points">'.__('Points Awarded', 'wpachievements').':</label>
    <div class="spinner-holder">
    ...
    if( class_exists(WPACHIEVEMENTS_WOOCOMMERCE) && class_exists(WPACHIEVEMENTS_WOOCOMMERCE_PAR) ){
      echo '<label for="wpachievements_achievements_data_wc_points">'.__('WooCommerce Points', 'wpachievements').': <small>(Optional)</small></label>
      <div class="spinner-holder">
 ...
 function wpachievements_image_box( $post ){
  ...
  while($file = readdir($handle)){
    if($file !== '.' && $file !== '..'){
      $ii++;
      echo '<span><input type="radio" name="achievement_badge" value="'.$path.$file.'" /><img src="'.$path.$file.'" alt="'.__('Achievement Image', 'wpachievements').' '.$ii.'" class="radio_btn" /></span>';
    }
  }
 ...
 function wpachievements_quest_how_box( $post ){
    ...
    if( !empty($cur_details) ){
      ...
      foreach( $cur_details as $cur_detail ){
        ...
        echo '<div id="quest_item_'.$ii.'">';
          ...
          echo '
          <span class="pullleft'.$extra_classes.'">
            <label for="wpachievements_achievements_data_event_'.$ii.'">'.__('Trigger Event:', 'wpachievements').'</label><br/>
            <select id="wpachievements_achievements_data_event_'.$ii.'" name="wpachievements_achievements_data_event_'.$ii.'" class="trigger_select" disabled title="'.__('This cannot be changed once the quest is created.', 'wpachievements').'">';
          ...
          if( function_exists(WPACHIEVEMENTS_LEARNDASH) ){
            ...
            echo '<span id="first_try" class="pullleft"'.$show.'>
              <label for="wpachievements_achievement_ld_first_try_'.$ii.'">'.__('First Attempt Only:', 'wpachievements').'</label>
          ...
          if(function_exists('is_multisite') && is_multisite()){
            echo '<span id="blog_limit" class="pullleft">
              <label for="wpachievements_achievement_blog_limit_'.$ii.'">'.__('Limit to Blog:', 'wpachievements').'</label>
          ...
          echo '<span id="ass_title"'.$show.'>
            <label for="wpachievements_achievement_bp_group_title_'.$ii.'">'.__('Group Title: <small>(Optional)</small>', 'wpachievements').'</label>
          ...
          echo '<span id="custom_event_details"'.$show.'>
            <label for="wpachievements_achievements_data_ach_id_'.$ii.'">'.__('Select Achievement:', 'wpachievements').'</label>
            <select id="wpachievements_achievements_data_ach_id_'.$ii.'" name="wpachievements_achievements_data_ach_id_'.$ii.'" disabled title="'.__('This cannot be changed once the quest is created.', 'wpachievements').'>';
              if( isset($cur_detail['ach_id']) ){
                echo '
                <optgroup label="'.__('Currently Selected', 'wpachievements').'">
          ...
          if( class_exists(WPACHIEVEMENTS_WOOCOMMERCE) ){
            ...
            echo '
            <span id="woo_order_limit"'.$show.'>
              <label for="wpachievements_achievement_woo_order_limit_'.$ii.'">'.__('Minimum Order Amount: <small>(Optional)</small>', 'wpachievements').'</label>
              <div class="spinner-holder">
                <div style="position:relative;">
                  <span id="wpa_woo_symbol">'.get_woocommerce_currency_symbol().'</span>
          ...
          echo '<label for="wpachievements_achievements_data_event_no_'.$ii.'">'.__('Number of Occurrences:', 'wpachievements').'</label>
          <div class="spinner-holder">
          ...
          echo '<a href="#" class="button_quest_remove'.$disabled.'">'.__('Remove Trigger', 'wpachievements').'</a>';
        echo '</div>';
      }
    } else{
      echo '<div id="quest_item_1">';
        ...
        echo '
        <span class="pullleft'.$extra_classes.'">
          <label for="wpachievements_achievements_data_event_1">'.__('Trigger Event:', 'wpachievements').'</label><br/>
        ...
        if( function_exists(WPACHIEVEMENTS_LEARNDASH) ){
          echo '<span id="first_try" class="pullleft">
            <label for="wpachievements_achievement_ld_first_try_1">'.__('First Attempt Only:', 'wpachievements').'</label>
            <select id="wpachievements_achievement_ld_first_try_1" name="wpachievements_achievement_ld_first_try_1">
              <option value="Disabled">'.__('Disabled', 'wpachievements').'</option>
              <option value="Enabled">'.__('Enabled', 'wpachievements').'</option>
            </select>
          </span>';
        }
        echo '<div class="clear"></div>';
          if(function_exists('is_multisite') && is_multisite()){
            echo '<span id="blog_limit" class="pullleft">
              <label for="wpachievements_achievement_blog_limit_1">'.__('Limit to Blog:', 'wpachievements').'</label>
          ...
          echo '<span id="ass_title">
            <label for="wpachievements_achievement_bp_group_title_1">'.__('Group Title: <small>(Optional)</small>', 'wpachievements').'</label>
          ...
          echo '<span id="post_id">
            <label for="wpachievements_achievements_data_post_id_1">'.__('Form ID: <small>(Optional)</small>', 'wpachievements').'</label>
          ...
          if( $achievement_query->have_posts() ){
            echo '<span id="custom_event_details" style="display:none;">
              <label for="wpachievements_achievements_data_ach_id_1">'.__('Select Achievement:', 'wpachievements').'</label>
              <select id="wpachievements_achievements_data_ach_id_1" name="wpachievements_achievements_data_ach_id_1">
                <option value="" selected>---------------- '.__('Select', 'wpachievements').' ----------------</option>';
                echo wpa_quest_achievement_list();
              echo '</select>
            </span>';
          }
          ...
          if( class_exists(WPACHIEVEMENTS_WOOCOMMERCE) ){
            ...
            <span id="woo_order_limit">
              <label for="wpachievements_achievement_woo_order_limit_1">'.__('Minimum Order Amount: <small>(Optional)</small>', 'wpachievements').'</label>
              <div class="spinner-holder">
                <div style="position:relative;">
                  <span id="wpa_woo_symbol">'.get_woocommerce_currency_symbol().'</span>
          ...
          echo '<label for="wpachievements_achievements_data_event_no_1">'.__('Number of Occurrences', 'wpachievements').':</label>
          <div class="spinner-holder">
        ...
        echo '
        <span class="pullleft'.$extra_classes.'">
          <label for="wpachievements_achievements_data_event_2">'.__('Trigger Event:', 'wpachievements').'</label><br/>
        ...
        if( function_exists(WPACHIEVEMENTS_LEARNDASH) ){
          echo '<span id="first_try" class="pullleft">
            <label for="wpachievements_achievement_ld_first_try_2">'.__('First Attempt Only:', 'wpachievements').'</label>
        ...
        echo '<div class="clear"></div>';
          if(function_exists('is_multisite') && is_multisite()){
            echo '<span id="blog_limit" class="pullleft">
              <label for="wpachievements_achievement_blog_limit_2">'.__('Limit to Blog:', 'wpachievements').'</label>
          ...
          echo '<span id="ass_title">
            <label for="wpachievements_achievement_bp_group_title_2">'.__('Group Title: <small>(Optional)</small>', 'wpachievements').'</label>
            <input type="text" id="wpachievements_achievement_bp_group_title_2" name="wpachievements_achievement_bp_group_title_2" value="" />
          </span>';
          echo '<span id="post_id">
            <label for="wpachievements_achievements_data_post_id_2">'.__('Form ID: <small>(Optional)</small>', 'wpachievements').'</label>
            <input type="text" id="wpachievements_achievements_data_post_id_2" name="wpachievements_achievements_data_post_id_2" value="" />
          </span>';
          echo '<span id="custom_event_details" style="display:none;">
            <label for="wpachievements_achievements_data_ach_id_2">'.__('Select Achievement:', 'wpachievements').'</label>
          ...
          if( class_exists(WPACHIEVEMENTS_WOOCOMMERCE) ){
            echo '
            <span id="woo_order_limit">
              <label for="wpachievements_achievement_woo_order_limit_2">'.__('Minimum Order Amount: <small>(Optional)</small>', 'wpachievements').'</label>
              <div class="spinner-holder">
                <div style="position:relative;">
                  <span id="wpa_woo_symbol">'.get_woocommerce_currency_symbol().'</span>
          ...
          echo '<label for="wpachievements_achievements_data_event_no_2">'.__('Number of Occurrences', 'wpachievements').':</label>
          <div class="spinner-holder">
  ...
  echo '<div class="event_details_holder">
    ...
    <div class="clear"></div><a href="#" class="button button-primary" id="quest_add">'.__('Add Another Trigger', 'wpachievements').'</a><span class="small_loader_icon"></span>
 ...
 */
 function wpachievements_quest_image_box( $post ){
  ...
  echo '<span id="no-image-links"><a href="#" id="achievement_image_pick">Select Image</a> &nbsp;|&nbsp; <input id="upload_image" type="text" name="upload_image" value="'.$cur_image.'" /><input class="button button-primary" id="upload_image_button" type="button" value="'.__('Upload Image', 'wpachievements').'" /></span>';
  echo '<div id="default-image-selection" style="display:none;">';
  $path = WPACHIEVEMENTS_URL . '/img/icons/';
  $handle = opendir( WPACHIEVEMENTS_PATH.'/img/icons/' );
  $ii=0;
 ...
 function get_wpachievements_quest_html(){
   if( isset($_POST['quest_count']) ){
     ...
     echo '<div id="quest_item_'.$count.'">';
       ...
       <div class="clear"></div><br/><div class="quest_sep"></div><br/>
       <span class="pullleft'.$extra_classes.'">
         <label for="wpachievements_achievements_data_event_'.$count.'">'.__('Trigger Event:', 'wpachievements').'</label><br/>
       ...
       if( function_exists(WPACHIEVEMENTS_LEARNDASH) ){
         echo '<span id="first_try" class="pullleft">
           <label for="wpachievements_achievement_ld_first_try_'.$count.'">'.__('First Attempt Only:', 'wpachievements').'</label>
       ...
       if(function_exists('is_multisite') && is_multisite()){
         echo '<span id="blog_limit" class="pullleft">
           <label for="wpachievements_achievement_blog_limit_'.$count.'">'.__('Limit to Blog:', 'wpachievements').'</label>
       ...
       echo '<span id="ass_title">
         <label for="wpachievements_achievement_bp_group_title_'.$count.'">'.__('Group Title: <small>(Optional)</small>', 'wpachievements').'</label>
       ...
       echo '<span id="post_id">
         <label for="wpachievements_achievements_data_post_id_'.$count.'">'.__('Form ID: <small>(Optional)</small>', 'wpachievements').'</label>
       ...
       if( $achievement_query->have_posts() ){
         echo '<span id="custom_event_details" style="display:none;">
           <label for="wpachievements_achievements_data_ach_id_'.$count.'">'.__('Select Achievement:', 'wpachievements').'</label>
       ...
       if( class_exists(WPACHIEVEMENTS_WOOCOMMERCE) ){
         echo '
         <span id="woo_order_limit">
           <label for="wpachievements_achievement_woo_order_limit_'.$count.'">'.__('Minimum Order Amount: <small>(Optional)</small>', 'wpachievements').'</label>
           <div class="spinner-holder">
             <div style="position:relative;">
               <span id="wpa_woo_symbol">'.get_woocommerce_currency_symbol().'</span>
       ...
       echo '<label for="wpachievements_achievements_data_event_no_'.$count.'">'.__('Number of Occurrences', 'wpachievements').':</label>
       <div class="spinner-holder">
      ...
      echo '<a href="#" class="button_quest_remove">'.__('Remove Trigger', 'wpachievements').'</a>';
     echo '</div>';
   }
   die();
 }
    ...
    <form name="wpachievements_ranks_data_form" method="post" id="add_rank_form">
      ...
      <table class="form-table">
        ...
        <tr valign="top">
          ...
          <td valign="middle"><input id="upload_image" type="text" name="upload_image" value="" /><input class="button button-secondary" id="upload_image_button" type="button" value="'.__('Upload Image', 'wpachievements').'" /></td>
        </tr>
      </table>
      <p class="submit">
 ...
 function wpachievements_show_extra_profile_fields( $user ){
   ...
   if( function_exists('is_super_admin') && is_super_admin() ){
    ...
    <?php if(!function_exists(WPACHIEVEMENTS_MYCRED)){ ?>
      <table class="form-table">
       <tr>
        <th><label for="wpa_points"><?php echo __('Points','wpachievements'); ?></label></th>
        <td>
 ...
 */
 ...
 function wpachievements_supported_plugins(){
   ?>
   <img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/logo.png'; ?>" alt="WPAchievements Logo" width="35" style="float:left;margin:6px 5px 0 0;" />
   <h2><?php echo __('WPAchievements - Supported Plugins', 'wpachievements'); ?></h2>
   ...
   <div id="plugin-support-images">
     <div class="item"><a href="http://wordpress.org/plugins/buddystream/" target="_blank" title="View: BuddyStream"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/buddystream.png'; ?>" alt="BuddyStream Banner" /></a></div>
     <div class="item"><a href="http://wordpress.org/plugins/buddypress/" target="_blank" title="View: BuddyPress"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/buddypress.png'; ?>" alt="BuddyPress Banner" /></a></div>
     <div class="item"><a href="http://simple-press.com/" target="_blank" title="View: Simple:Press"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/simplepress.png'; ?>" alt="Simple:Press Banner" /></a></div>
     <div class="item"><a href="http://wordpress.org/plugins/bbpress/" target="_blank" title="View: bbPress"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/bbpress.png'; ?>" alt="bbPress Banner" /></a></div>
     <div class="item"><a href="http://www.learndash.com/" target="_blank" title="View: LearnDash"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/learndash.png'; ?>" alt="LearnDash Banner" /></a></div>
     <div class="item"><a href="http://wordpress.org/plugins/gd-star-rating/" target="_blank" title="View: GD Star Rating"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/gd-star-rating.png'; ?>" alt="GD Star Rating Banner" /></a></div>
     <div class="item"><a href="http://www.wpcourseware.com/" target="_blank" title="View: WP-Courseware"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/wp-courseware.png'; ?>" alt="WP-Courseware Banner" /></a></div>
     <div class="item"><a href="http://www.gravityforms.com/" target="_blank" title="View: Gravity Forms"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/gravity-forms.png'; ?>" alt="Gravity Forms Banner" /></a></div>
     <div class="item"><a href="http://myarcadeplugin.com/" target="_blank" title="View: MyArcadePlugin"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/myarcadeplugin.png'; ?>" alt="MyArcadePlugin Banner" /></a></div>
     <div class="item"><a href="http://exells.com/shop/arcade-plugins/myarcadecontest/" target="_blank" title="View: MyArcadeContest"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/myarcadecontest.png'; ?>" alt="MyArcadeContest Banner" /></a></div>
     <div class="item"><a href="http://wordpress.org/plugins/mycred/" target="_blank" title="View: myCRED"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/mycred.png'; ?>" alt="myCRED Banner" /></a></div>
     <div class="item"><a href="http://codecanyon.net/item/userpro-user-profiles-with-social-login/5958681" target="_blank" title="View: UserPro"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/userpro.png'; ?>" alt="UserPro Banner" /></a></div>
     <div class="item"><a href="http://wordpress.org/plugins/cubepoints/" target="_blank" title="View: CubePoints"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/cubepoints.png'; ?>" alt="CubePoints Banner" /></a></div>
     <div class="item"><a href="http://wordpress.org/plugins/woocommerce/" target="_blank" title="View: BuddyStream"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/woocommerce.png'; ?>" alt="WooCommerce Banner" /></a></div>
     <div class="item"><a href="http://wordpress.org/plugins/wp-e-commerce/" target="_blank" title="View: WP e-Commerce"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/wp-e-commerce.png'; ?>" alt="WP e-Commerce Banner" /></a></div>
     <div class="item"><a href="http://wordpress.org/plugins/jigoshop/" target="_blank" title="View: Jigoshop"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/jigoshop.png'; ?>" alt="Jigoshop Banner" /></a></div>
     <div class="item"><a href="http://wordpress.org/plugins/wp-favorite-posts/" target="_blank" title="View: WP Favorite Posts"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/wp-favorite-posts.png'; ?>" alt="WP Favorite Posts Banner" /></a></div>
     <div class="item"><a href="http://wordpress.org/plugins/invite-anyone/" target="_blank" title="View: Invite Anyone"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/invite-anyone.png'; ?>" alt="Invite Anyone Banner" /></a></div>
     <div class="item"><img src="<?php echo WPACHIEVEMENTS_URL . '/assets/img/banners/wordpress.png'; ?>" alt="WordPress Banner" /></div>
   </div>
   <?php
 }
  ...
  if ( get_option( 'wpachievements_rating_div' ) == "no" ) {
    ...
    if ( $diff_intrval >= 7 ) {
      echo '<div class="updated notice wpachievements_fivestar">
        <p>Awesome, you\'ve been using <strong>WPAchievements</strong> for a while. May we ask you to give it a <strong>5-star</strong> rating on Codecanyon?
          <br /><strong>Your WPAchievements Team</strong>
          <ul>
            <li><a href="https://codecanyon.net/downloads" class="thankyou" target="_new" title="Ok, you deserved it" style="font-weight:bold;">Ok, you deserved it</a></li>
...
function wpachievements_documentation(){
 ...
 <div id="wpa_latest_products">
   ...
   echo '<div class="wpa_product_item">
      <div class="wpa_product_links">
        <img src="'.WPACHIEVEMENTS_URL . '/assets/img/magazine_arcade.jpg'.'" width="80" height="80" alt="" class="wpa_pull_left">
 ...
 function wpachievements_remove_rank_ajax_callback(){
   if (isset($_POST['wpachievements_rank_remove']) != '') {
    if((int)$_POST['wpachievements_rank_remove']==0){
      echo '<div class="error"><p><strong>'. __('A rank name is needed for users with 0 points!<br /><br />Click the edit link to edit this rank.', 'wpachievements') .'</strong></p></div>';
    } else{
 ...
 function wpachievements_update_rank_ajax_callback(){
   if(isset($_POST['wpachievements_ranks_data_rank']) != ''){
     ...
     if( $wpachievements_ranks_data_rank == '' || $_POST['wpachievements_ranks_data_points'] == '' ){
       echo '<div class="error"><p><strong>'. __('Rank name or points cannot be empty!', 'wpachievements') .'</strong></p></div>';
       ...
       echo '<div class="error"><p><strong>'. __('Please enter only positive integers for the points!', 'wpachievements') .'</strong></p></div>';
     } else{
       if( isset($ranks[$wpachievements_ranks_data_points]) ){
         echo '<div class="updated"><p><strong>'. __('Rank Updated', 'wpachievements') .'</strong></p></div>';
       } else{
         echo '<div class="updated"><p><strong>'. __('Rank Added', 'wpachievements') .'</strong></p></div>';
       }
 ...
 */
 ...
 /*function wpachievements_license_notice(){
   $license_key = wpachievements_get_site_option('wpachievements_license_key');
   wp_enqueue_script( 'NoticeScript', WPACHIEVEMENTS_URL . '/assets/js/notice-script.js' );
   if( empty($license_key) ){
     echo '<div class="updated"><p>
     <strong><p style="display:inline-block;">'. __('Enter Your WPAchievements Purchase Code:', 'wpachievements') .'&nbsp;</p></strong>
