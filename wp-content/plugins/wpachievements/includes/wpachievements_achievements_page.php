...
wp_enqueue_style( 'wpachievements-achievements-page-style', WPACHIEVEMENTS_URL . '/assets/css/page-style.css' );
...
<div id="wpa_list_holder">
  <div class="wpa_list_hold" id="wpa_list_quests"'.$questsShow.'>
    <h2 class="wpa_heading_title"><span id="quest_icon"></span>'.sprintf(__('Our %sQuests%s', 'wpachievements'), '<span>','</span>').'</h2>
              ...
              $quest_type = get_post_meta( $quest_ID, '_achievement_type', true );
                ...
                echo '<div class="achive_tab" id="quest_tab_'.$ii.'"'; if($ii==1){echo 'style="display:block;"';} echo '>
                  <div class="wpa_image_block">';
                    if( is_user_logged_in() ){
                      ...
                      if( is_array($userquests) && in_array($quest_ID, $userquests) ){
                        echo '<div class="ach_point unlocked">'.__('Unlocked', 'wpachievements').'</div>';
                      } else{
                  ...
                  echo '<div class="wpa_info_block">';
                    ...
                    <h3>'.__('How do I get this?', 'wpachievements').'</h3>';
                      ...
                      foreach( $cur_details as $cur_detail ){
                        ...
                        if( !array_key_exists($cur_detail['type'],$already_counted) || $cur_detail['type'] == 'wpachievements_achievement' ){
                          if( $cur_detail['type'] == 'wpachievements_achievement' ){
                            if( $iiii != 1 ){
                              $quests[$cur_detail['type']] = ', '.__('Unlock achievement: ', 'wpachievements').' '.get_achievement_name($cur_detail['ach_id']);
                            } else{
                              $quests[$cur_detail['type']] = __('Unlock achievement: ', 'wpachievements').' '.get_achievement_name($cur_detail['ach_id']);
                            }
                          } else{
...
wp_reset_postdata();
  ...
  echo '<div class="wpa_list_hold" id="wpa_list_quests"'.$achievementsShow.'>
    ...
    <h2 class="wpa_heading_title"><span id="achievement_icon"></span>'.sprintf(__('Our %sAchievements%s', 'wpachievements'), '<span>','</span>').'</h2>
    <div class="wpa_list_inner">
      <div class="wpa_list_inner_info">
        <div id="wpa_info">';
          $ii=0;
                ...
                echo '<div class="achive_tab" id="ach_tab_'.$ii.'"'; if($ii==1){echo 'style="display:block;"';} echo '>
                  ...
                  <div class="wpa_info_block">
                    ...
                    if( $ach_type == 'custom_trigger' ){
                      echo '<p style="margin-top:0;">'.__('Get this achievement for', 'wpachievements').' '. $ach_trigger_desc .'</p>';
                    } else{
                      echo '<p style="margin-top:0;">'.__('Get this achievement', 'wpachievements').' '. achievement_Desc($ach_type,'',$ach_occurences,'',$ach_data) .'</p>';
                    }
                  echo '</div>';
                echo '</div>';
              } else{
