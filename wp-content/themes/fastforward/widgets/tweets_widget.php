...
class widget_tweets extends WP_Widget {
    function widget_tweets() {
        ...
        parent::WP_Widget( 'widget_cosmo_tweets' , __('Latest tweets','cosmotheme') , $widget_ops );
    ...
    function form($instance) {
        ...
        if( isset($instance['APISecret']) ){
            $APISecret = esc_attr($instance['APISecret']);
        }else{
            $APISecret = null;
        }
        ...
        <p>
            <span class="hint"><?php echo sprintf(__( 'Now you need to do it to create an application in %s Twitter Apps %s and fill the requirements there. Once finished you will have your API key, API secret, access token and access token secret.' , 'cosmotheme' ), '<a href="https://dev.twitter.com/apps">', '</a>' ); ?></span>

        </p>

        <p>
            <label for="<?php echo $this->get_field_id( 'APIKey' ); ?>"><?php _e( 'API key' , 'cosmotheme' ); ?>:</label>
            <input id="<?php echo $this->get_field_id( 'APIKey' ); ?>"   name="<?php echo $this->get_field_name('APIKey'); ?>" type="text" value="<?php echo $APIKey; ?>" />
        </p>

        <p>
            <label for="<?php echo $this->get_field_id( 'APISecret' ); ?>"><?php _e( 'API secret' , 'cosmotheme' ); ?>:</label>
            <input id="<?php echo $this->get_field_id( 'APISecret' ); ?>"   name="<?php echo $this->get_field_name('APISecret'); ?>" type="text" value="<?php echo $APISecret; ?>" />
        </p>
    ...
    function update( $new_instance, $old_instance) {
        ...
        $instance['APISecret']   = strip_tags($new_instance['APISecret']);
        $instance['accessToken']   = strip_tags($new_instance['accessToken']);
        $instance['accessTokenSecret']   = strip_tags($new_instance['accessTokenSecret']);
        return $instance;
    }

    function the_tweets( $username , $number, $dynamic, $APIKey, $APISecret, $accessToken, $accessTokenSecret  ){
    ...
    function get_tweets( $feed_info , $classes = '' , $before = "<div class='tweet_item'><p>" , $after = "</p></div>", $static_class = '' ){
        ...
        */
        ...
        if(false === ($tweets = get_transient($transName) ) ||  get_transient($transName) == '') :
            ...
            get_template_part('vendors/twitteroauth/twitteroauth');

            $twitterConnection = new TwitterOAuth(
                $feed_info['APIKey'],	// API Key
                $feed_info['APISecret'],   	// API secret
        ...
        if($tweets){
            $Tweets_limits = null;
            foreach($tweets as $id =>$t) : $Tweets_limits++ ;if( $numTweets >= $Tweets_limits) : ?>
    ...
    function widget($args, $instance) {
        ...
        self::the_tweets( $username , $number, $dynamic, $APIKey, $APISecret, $accessToken, $accessTokenSecret );
