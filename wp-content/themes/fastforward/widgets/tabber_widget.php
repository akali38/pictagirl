<?php
    class Tabber_Widget extends WP_Widget {
        public function __construct() {
            parent::__construct(
                'monstro_tabber_widget',
                __('Tabber Widget', 'monstrotheme'),
                array( 'description' => __( 'Display Tabber Widget', 'monstrotheme' ), )
            );
        }
        ...
        ?>
            ...
            <ul class="tabs-controller">
                ...
                ?>
                        <li><a class="active" href="#tabber_hot_posts_panel"><?php _e( 'Hot' , 'monstrotheme' )?></a></li>
                        <li><a href="#tabber_new_posts_panel"><?php _e( 'New' , 'monstrotheme' )?></a></li>
                <?php
                    }else{
                        ?><li class="active"><a href="#tabber_new_posts_panel"><?php _e( 'New' , 'monstrotheme' )?></a></li><?php
                    }

                    if ( options::get_value( 'blog_post' , 'comments_type' ) != 'fb_comments' ) {
                ?>
                        <li><a href="#tabber_comments_panel"><?php _e( 'Replies' , 'monstrotheme' )?></a></li>
                <?php
                    }
                ?>
                <li><a href="#tabber_tags_posts_panel"><?php _e( 'Tags' , 'monstrotheme' )?></a></li>
            </ul>
            <!-- panel hot posts -->
            <?php
                if( options::logic( 'likes' , 'enb_likes' ) ){
            ...
            <div id="tabber_new_posts_panel" class="tab_menu_content tabs-container <?php  echo $nclasses; ?>">
                <?php
                    ...
                    if( $query -> have_posts() ){
                        ...
                        echo '<p>' . __( 'Sorry, no posts found.' , 'monstrotheme' ) . '</p>';
                    }
                                    ...
                                    <article class="row">
                                        <div class="large-4 small-12 columns">
                                            <a class="entry-img" href="<?php echo get_permalink( $comment -> comment_post_ID ) . '#comment-' . $comment -> comment_ID; ?>">
                                                <?php $size = image::asize( 'thumbnail' ); echo get_avatar( $comment -> user_id , $size[0] , DEFAULT_AVATAR );  ?>
                                            </a>
                                        </div>
            ...
            <div id="tabber_tags_posts_panel" class="tab_menu_content tabs-container">
                <?php
                    ...
                    if( !empty( $tags ) && is_array( $tags ) ){
                        ...
                        echo '<p>' . __( 'There are no tags.' , 'monstrotheme' ) . '</p>';
                    }
                ?>
            </div> 
        </div>           
        ...
        function form($instance) {
            ...
            $instance       = wp_parse_args( (array) $instance, array( 'title' => '' , 'nr_hot_posts' => 10 , 'period' => 7 , 'nr_new_posts' => 10 , 'nr_comments' => 10 , 'nr_tags' => '', 'img_size' => 'large_img') );
    ...
    ?>
            ...
            <p>
                <label for="<?php echo $this->get_field_id('nr_hot_posts'); ?>"><?php _e( 'Number of hot posts' , 'monstrotheme' ) ?>:
            ...
            <p>
                <label for="<?php echo $this->get_field_id('period'); ?>"><?php _e( 'Period of hot posts' , 'monstrotheme' ) ?>:
            ...
            <p>
                <label for="<?php echo $this->get_field_id('nr_new_posts'); ?>"><?php _e( 'Number of new posts' , 'monstrotheme' ) ?>:
			...
			<p>
                <label for="<?php echo $this->get_field_id('nr_tags'); ?>"><?php _e( 'Number of tags' , 'monstrotheme' ) ?>:
					...
					<span class="hint"><?php _e('Leave blank to show all tags','monstrotheme' ) ?></span>
                </label>
            </p>
            <p>
