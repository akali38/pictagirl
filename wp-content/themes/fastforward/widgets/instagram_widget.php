...
class Instagram_Widget extends WP_Widget {
    public function __construct() {
        parent::__construct(
            'monstro_instagram_widget',
            __('Instagram Photos', 'monstrotheme'),
            array( 'description' => __( 'Instagram Photos', 'monstrotheme' ), )
        );
    }
		...
		if(!empty($results['data'])){
			foreach($results['data'] as $item){
				if($picture_number == 0){
					...
					echo sprintf(__('%s Please set the Number of images to show within the widget %s','monstrotheme'),'<strong>', '</strong>');
					break;
				}
				...
				if($show_likes){
					if(!empty($item['likes']['count'])){
						echo "<p class='instagram_likes'>".__('Likes:','monstrotheme')." <span class='likes_count'>".$item['likes']['count']."</span></p>";
					}
				}
				echo '</div>';
				if($show_caption){
			...
			echo "<strong>".__('The user currently does not have any images...','monstrotheme')."</strong>";			
		}
		echo $after_widget;
	}
	...
	function form( $instance ) {
		if ( $instance ) {
			...
			$title = __( 'Title', 'monstrotheme' );
			$username = __( 'Username', 'monstrotheme' );
			$access_token = __( 'Access Token', 'monstrotheme' );
			$user_id = __( 'User ID', 'monstrotheme' );
			$picture_size = 'thumbnail';
			$picture_number = 0;
			$show_likes = false;
			$show_caption = false;
		...
		<p>
		<label for="<?php echo $this->get_field_id('user_id'); ?>"><?php _e('User ID:','monstrotheme'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('access_token'); ?>"><?php _e('Access Token:','monstrotheme'); ?></label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id('picture_number'); ?>"><?php _e('Number of Images:','monstrotheme'); ?></label> 
			<select id="<?php echo $this->get_field_id('picture_number'); ?>" name="<?php echo $this->get_field_name('picture_number'); ?>">
					<option value="0"><?php _e('Select Number','monstrotheme'); ?></option>
				<?php for($i=1;$i<11;$i++):?>
		...
		<p>
			<label for="<?php echo $this->get_field_id('picture_size'); ?>"><?php _e('Picture Size:','monstrotheme'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('link_images'); ?>"><?php _e('Link images to full image:','monstrotheme'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('show_likes'); ?>"><?php _e('Show Likes:','monstrotheme'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('show_caption'); ?>"><?php _e('Show Caption:','monstrotheme'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('debug_mode'); ?>"><?php _e('Debug Mode:','monstrotheme'); ?></label> 
		...
		<?php
			$token_msg = sprintf(__('If you do not have an ID or access token, please visit %s Get Access token %s to receive a valid token','monstrotheme'),'<a href="'.$token_url.'" target="_blank">', '</a>');
		?>
